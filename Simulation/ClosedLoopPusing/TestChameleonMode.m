...
miqp_steps = 5;
fom_steps = 10;
hybrid_modes = [1, 1, 2, ones(1, fom_steps - 3); 1, 2, ones(1, fom_steps - 2); 1, 1, 3, ones(1, fom_steps - 3)];
%% MPC Parameters and Setup
Q_MPC = 10 * diag([1,1,.1,0]); % 10 * diag([1,10,10,0]);
Q_MPC_final = 0 * diag([1,1,.1,0]); % 200 * diag([.1,10,1,0]);
R_MPC = .5 * diag([1,1]);
...
% solvers{1, 3} = FOMSolver(hybrid_states_map, Q_MPC, Q_MPC_final, R_MPC, u_lower_bound, u_upper_bound, x_lower_bound, x_upper_bound, h_opt, [hybrid_modes; ], 1); % FOM with chameleon
...
path_name = 'SimulationResults/ChameleonModeTest';
if  exist(path_name, 'dir') ~= 7
    mkdir(pwd, path_name);
end
video_names = {strcat(path_name, '/fom_video'), strcat(path_name, '/fom_with_chameleon_video'), ...
%     strcat(path_name, '/fom_with_chameleon_and_extra_mode_video') ...
    };
